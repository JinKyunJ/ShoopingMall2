document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener('click', function (event) {
    if (event.target.classList.contains('cart-btn')) {
      ...
      // 여기에 '담기' 버튼 클릭 시 수행할 로직 추가
      console.log("Product added to cart:", product);
      // 예를 들어, 모달 표시 함수 호출 등을 추가할 수 있음
    }
  });
  ...
  async function fetchProducts(type) {
    ...
    if (type === "sale") {
      ...
      result = products
          ...
          .sort((a, b) => b.product.comments.length - a.product.comments.length);
    } else if (type === "popular") {
      result = products
          .filter(({ likeUser }) => likeUser.length > 0)
          .sort((a, b) => b.likeUser.length - a.likeUser.length);
    } else if (type === "salad") {
  ...
  async function fetchCommentProducts() {
    try {
      ...
      const commentProductList = document.getElementById("comment-product-wrapper");
  ...
  async function fetchPopularProducts() {
    try {
      ...
      const popularProductList = document.getElementById("popular-product-wrapper");
